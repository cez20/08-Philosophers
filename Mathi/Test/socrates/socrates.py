...
def socrates(bin_path, philo_num):
    ...
    if os.path.isfile(f"{bin_path}/Bin/philo") and (
        philo_num == 0 or philo_num == 1
    ):
        print(f"\n{bcolors.OKBLUE}---------- PHILO ----------------{bcolors.ENDC}\n")
        test_program(f"{bin_path}/Bin/philo")
    if os.path.isfile(f"{bin_path}/Bin/philo_bonus") and (
        philo_num == 0 or philo_num == 2
    ):
        print(f"\n{bcolors.OKBLUE}---------- PHILO_BONUS ----------{bcolors.ENDC}\n")
        test_program(f"{bin_path}/Bin/philo_bonus")
    if config.FAIL == 1:
        return 1
    else:
        return 0
